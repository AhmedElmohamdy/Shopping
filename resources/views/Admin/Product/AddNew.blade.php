                    ...
                    <div class="contact-form">
                        <form method="POST" id="ProductForm" action="{{route('Prodect.Save')}}" enctype="multipart/form-data" >
                            ...
                            <p>
                                ...
                                <input type="text" style="width: 100%" placeholder=@lang('messages.Product Name') name="product_name" id="product_name" required>
                                @if ($errors->has('product_name'))
                                <span style="color: red;">{{ $errors->first('product_name') }}</span>
                            @endif
                            </p>
                            <p>
                                <input type="text" style="width: 100%" placeholder=@lang('messages.ProdactNameInArabic') name="ProdactNameAr" id="ProdactNameAr" required>
                                @if ($errors->has('ProdactNameAr'))
                                <span style="color: red;">{{ $errors->first('ProdactNameAr') }}</span>
                            @endif
                            </p>
                            <p style="display: flex">
                                <input type="number" style="width: 20%" class="mr-3" placeholder=@lang('messages.Product Price') name="product_price" id="product_price" min="0" required>
                                @if ($errors->has('product_price'))
                                <span style="color: red;">{{ $errors->first('product_price') }}</span>
                            @endif


                                <input type="number" style="width: 20%" placeholder=@lang('messages.Product Quantity') name="product_quantity" id="product_quantity" min="0" required>
                            ...
                            <button type="submit" id="submitButton" class="btn btn-success">@lang('messages.AddNewProdect')</button>
