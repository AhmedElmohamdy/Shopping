...
<div class="container mt-4">
    <div class="left-center">
      <a href="{{route('Category.AddNew')}}" class="btn btn-secondary" >@lang('messages.Add New Category')  </a>
    </div>
    <table id="myTable" class="display table table-striped table-bordered" style="width:100%">
        <thead>
          <tr>
            <th scope="col">@lang('messages.Id')</th>
            <th scope="col">@lang('messages.Category Name')</th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ session('locale') == 'en' ? $Cat ->category_name: $Cat ->CategoryNameAr }}</td>
                <td>{{ session('locale') == 'en' ? $Cat ->description : $Cat->DescriptionAr }}</td>
                <td><img src="{{ asset($Cat ->image_name) }}" width="100" height="100" alt="Product Image"></td>
                <td>{{$Cat ->created_at}}</td>
                <td>{{$Cat ->updated_at}}</td>
                <td>
                    <a href="{{route('Category.edit',$Cat ->id)}}" class="btn btn-primary" >@lang('messages.Edit')  </a>

                    <form style="display: inline" method="POST" action="{{route('Category.Delete',$Cat ->id)}}">
                      @csrf
                      @method('DELETE')
